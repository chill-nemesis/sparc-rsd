...
class MagneticDipole:
    def __init__(
        ...
        self.magnetic_moment = self.magnetization * self.volume

        self.reset()

    @property
    def global_magnetization(self):
        return np.dot(self.orientation, self.magnetization)

    def magnetic_torque(self, B):
        return np.cross(self.magnetic_moment, B)

    def magnetic_force(self, GradB):
        return np.dot(GradB, self.magnetic_moment)
