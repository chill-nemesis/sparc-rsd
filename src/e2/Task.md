...
- Once you implemented the IK solver, feel free to play around with the values for alpha, and/or the target position What happens if you use a large alpha (e.g., .8)? What happens, if you choose (0, 0, 0) as the target position for the end effector? Or something far away, e.g., (1000, 0, 0)? Or simply try the other target position ([0, 5, 4.5] instead of [5, 0, 4.5]. Although just a simple rotation, something rather spectacular should happen...)
